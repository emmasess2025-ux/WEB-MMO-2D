...
ws.onmessage = (event) => {
    ...
    if (data.type === 'auth_error') {
        ...
        bpXP = data.totalXP;
        // Optionally show a flying text or notification
        showNotification(? + BP XP!);
        renderBattlePass();
    } else if (data.type === 'bp_premium_unlocked') {
        bpPremium = true;
        showNotification(? PREMIUM PASS UNLOCKED!);
        renderBattlePass();
        ...
        let rw = data.reward;
        let rewardName = rw.type === 'item' ? rw.id : ${rw.amount} ;
...
function updateTaskTimers() {
    ...
    timers.forEach(timerEl => {
        ...
        if (diffMs <= 0) {
            ...
            // Matem�ticas simples para sacar Horas, Minutos y Segundos
            const totalSeconds = Math.floor(diffMs / 1000);
            const hours = Math.floor(totalSeconds / 3600).toString().padStart(2, '0');
            const minutes = Math.floor((totalSeconds % 3600) / 60).toString().padStart(2, '0');
            const seconds = (totalSeconds % 60).toString().padStart(2, '0');

            timerEl.innerText = \? Disponible en \:\:\\;
        }
    });
}
...
if (buyPremiumBtn) {
    buyPremiumBtn.addEventListener('click', () => {
        ...
        if (confirm(\Unlock Premium Pass for \ Argems?\)) {
...
function renderBattlePass() {
    ...
    if (nextXpReq > 0) {
        ...
        bpXpText.innerText = \\ / \ XP\;
    } else {
        bpXpBar.style.width = '100%';
        bpXpText.innerText = \\ XP (MAX)\;
    }
}
...
function createBpBox(rewardObj, track) {
    ...
    label.innerText = reward.type === 'item' ? reward.id : \\ \\;
